...
def command_out(command):
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)  # command must be a list
...
def mitre_lab_17_api(request):
    if request.method == "POST":
        ...
        # Optional: validate ip input; replace with proper validation logic as needed
        import ipaddress
        try:
